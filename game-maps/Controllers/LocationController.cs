...
namespace game_maps.Controllers
{
    ...
    {
        [HttpGet("{slug}")]
        public async Task<IList<LocationCateogryDto>> GetAllLocations(string slug)
        {
            var id = User?.FindFirstValue(ClaimTypes.NameIdentifier);
            return await locationService.GetAllLocations(slug, id);
        }
        ...
        [HttpGet("search")]
        public async Task<IList<LocationSearchDto>> SearchForLocations(string slug, string search)
        {
            return await locationService.SearchInLocation(slug, search);
        }
    }
}
